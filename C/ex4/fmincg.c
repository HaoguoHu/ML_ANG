...
{
	int i,j, success = 0,costFuncCount=0,lineSearchFuncCount=0;
	double ls_failed,Cost1,d1,z1,f0,Cost2,d2,f3,d3,z3,limit,z2,A,B,C;
	int ndim = M1*N1+M2*N2;
	double grad1[ndim],s[ndim],x0[ndim],df0[ndim],grad2[ndim],tmp[ndim];


	ls_failed = 0;

	if(costFuncCount >= maxCostCalls) return 1; else costFuncCount++;
	Cost1 = nnCostFunction(M,N,M1,N1,M2,N2, nn_params, X, y, lambda, grad1);
//	printf("Cost1 = %lf \n");
//		getchar();
	...
	while(1)
	{
		...
		for(i=0;i<ndim;i++){
			nn_params[i] = nn_params[i] + (z1)*s[i];
		}
		if(costFuncCount >= maxCostCalls) return 1; else costFuncCount++;
		Cost2 = nnCostFunction(M,N,M1,N1,M2,N2, nn_params, X, y, lambda, grad2);
